...
//CARD 3--THE FUTURE 
function getRandom3(num, exclude1, exclude2) {
    let randomNumber3;
    do {
        randomNumber3 = Math.floor(Math.random() * num);
    } while (randomNumber3 === exclude1 || randomNumber3 === exclude2);
    return randomNumber3
}
//need to generate random THREE CARD SPREADS/READINGS 
let randomNumber = getRandom(tarotDeck.length);
let randomNumber2 = getRandom2(tarotDeck.length, randomNumber);
let randomNumber3 = getRandom3(tarotDeck.length, randomNumber, randomNumber2);

//display the Back of the Card Deck initially 
cards.forEach(card => {
    card.src = "imgs/78CardBack.jpeg";
});

//creating functions to display a card 
function displayCard(index, cardElementId, descriptionElementId) {
    const currentCard = tarotDeck[index];
    document.getElementById(cardElementId).innerHTML = '<img src="imgs/cards${currentCard.image}.jpeg" alt="${currentCard.title}">'; document.getElementById(cardElementId).innerHTML = `<img src="imgs/cards${currentCard.image}.jpeg" alt="${currentCard.title}">`;
    document.getElementById(descriptionElementId).textContent = currentCard.description;
